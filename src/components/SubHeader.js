...
function SubHeader({ onBookSelect, setSearchResult }) {
  ...
  const handleSearch = async () => {
    try {
      const response = await Axios.get(`https://libraryserver-xey1.onrender.com/searchBooks?query=${searchInput}`);
  ...
  const handleInputChange = async (e) => {
    ...
    try {
      const response = await Axios.get(`https://libraryserver-xey1.onrender.com/searchBooks?query=${e.target.value}`);
